...
async function handleDataUrl(
  ...
  try {
    const savedPath = await saveImageToTemp(parsed.data, parsed.mime, sessionID);
    log(`Saved pasted image to temp file: ${savedPath}`);
    return { path: savedPath, mime: parsed.mime, partId: filePart.id, sessionID };
  } catch (error) {
    log(
...
async function processImagePart(
  ...
  if (url.startsWith("data:")) return handleDataUrl(url, filePart, log, sessionID);
